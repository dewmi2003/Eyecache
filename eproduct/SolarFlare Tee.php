...
// Product Data
$product = [
    "id" => 1, // assign a unique product ID
    "name" => "SolarFlare Tee",
  ...
  <style>
    * { box-sizing: border-box; margin:0; padding:0; }
    body {
        ...
        background:#111; color:#eee; line-height:1.6;
        scroll-behavior:smooth;
    }
    a{ color:#ff3c78; text-decoration:none;}
    img{ max-width:100%; display:block; transition: transform 0.4s, box-shadow 0.4s;}
    .container{
        width:90%; max-width:1200px; margin:100px auto 40px auto;
    ...
    .sizes button{
        padding:12px 22px; margin:5px; border:2px solid #333; background:#fff;
        cursor:pointer; font-weight:500; border-radius:5px; font-size:15px; transition:0.3s;
    }
    ...
    .add-to-cart input{
        width:65px; text-align:center; padding:7px; border:1px solid #ccc; border-radius:5px; font-size:15px;
    }
    .add-to-cart button{
        background:#ff3c78; color:#fff; border:none; padding:13px 28px;
        cursor:pointer; font-size:16px; border-radius:6px; font-weight:600; transition:0.3s;
    }
    .add-to-cart button:hover{ opacity:0.9; transform:translateY(-2px);}
    footer{
        background:#111; color:#bbb; text-align:center; padding:1.5rem 0;
        font-size:1rem; margin-top:auto; border-top:1px solid #222;
    }
  </style>
</head>
<body>
...
<script>
  ...
  document.getElementById("addToCartBtn").addEventListener("click", () => {
    ...
    let color = selectedColor.getAttribute("title");
    ...
    fetch("add_to_cart.php", {
      ...
      body: `user_id=${user_id}&product_id=${product_id}&size=${size}&color=${color}&quantity=${qty}`
    })
    .then(res => res.text())
    .then(data => {
